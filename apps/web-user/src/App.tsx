        ...
        element={
          user ? (
            <div className="n8n-shell">
              <div className="n8n-banners">
                <div className="n8n-banner-inner">
                  <Sparkles size={13} />
                  <span>AI训练数据工厂 · LLM数据集生成平台</span>
                  <Tag color={isAdmin ? 'green' : 'blue'} size="small">{isAdmin ? '管理员' : '普通用户'}</Tag>
                </div>
              </div>
              <div className={clsx('n8n-sidebar', sidebarCollapsed && 'n8n-sidebar--collapsed')}>
                <div className="n8n-sidebar-header">
                  <div className="n8n-workspace-row">
                    <Avatar color="blue" size="extra-small">L</Avatar>
                    {!sidebarCollapsed && <span className="n8n-workspace-name">数据工厂</span>}
                  </div>
                  <button className="n8n-icon-btn" onClick={() => setSidebarCollapsed((c) => !c)}><LayoutDashboard size={14} /></button>
                </div>
                <nav className="n8n-sidebar-nav">
                  {visibleUserPages.map((page) => (
                    <button key={page.route} className={clsx('n8n-nav-item', activeNav === page.route && 'n8n-nav-item--active')} onClick={() => navigate(page.route)} title={page.label}>
                      <page.icon size={16} strokeWidth={1.7} />
                      {!sidebarCollapsed && <span>{page.label}</span>}
                    </button>
                  ))}
                  {isAdmin && (<><div className="n8n-nav-divider" />{adminPages.map((page) => (<button key={page.route} className={clsx('n8n-nav-item', activeNav === page.route && 'n8n-nav-item--active')} onClick={() => navigate(page.route)} title={page.label}><page.icon size={16} strokeWidth={1.7} />{!sidebarCollapsed && <span>{page.label}</span>}</button>))}</>)}
                </nav>
                <div className="n8n-sidebar-footer">
                  {!sidebarCollapsed && <span className="n8n-user-email">{user.email}</span>}
                  <button className="n8n-icon-btn" onClick={() => void handleLogout()} title="退出"><LogOut size={14} /></button>
                </div>
              </div>
              <div className="n8n-header">
                <div className="n8n-header-left">
                  <span className="n8n-header-title">{visiblePages.find((p) => p.route === activeNav)?.label ?? '工作台'}</span>
                  <span className="n8n-header-sep">·</span>
                  <span className="n8n-header-caption">{visiblePages.find((p) => p.route === activeNav)?.caption ?? ''}</span>
                </div>
                <div className="n8n-header-actions">
                  ...
                  <Button size="small" theme="solid" type="primary" icon={<CirclePlus size={14} />} onClick={() => navigate('/console/planning')}>新建任务</Button>
                </div>
              </div>
              ...
              <div className="n8n-aside">
                <div className="n8n-aside-title">运行状态</div>
                <div className="n8n-aside-stat"><span>任务总数</span><strong>{runtime?.datasetCount ?? 0}</strong></div>
                <div className="n8n-aside-stat"><span>队列深度</span><strong>{runtime?.queueDepth ?? 0}</strong></div>
                <div className="n8n-aside-stat"><span>建议</span><strong style={{fontSize:11}}>{runtime?.datasetCount ? '继续推进' : '新建任务'}</strong></div>
                <div className="n8n-aside-pipeline-label">流水线阶段</div>
                {['主题结构','问题生成','答案内容','质量评估','导出交付'].map((stage, i) => (
                  <div key={i} className="n8n-aside-stage">
                    <span className="n8n-stage-num">{i + 1}</span>
                    <span className="n8n-stage-name">{stage}</span>
                  </div>
                ))}
              </div>
            </div>
