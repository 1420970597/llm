        ...
        element={
          user ? (
            <div className="plane-shell">
              <div className={clsx('plane-sidebar', sidebarCollapsed && 'plane-sidebar--collapsed')}>
                <div className="plane-sidebar-header">
                  <div className="plane-workspace-badge">
                    <Avatar color="blue" size="extra-small">L</Avatar>
                    {!sidebarCollapsed && <span className="plane-workspace-name">企业数据工厂</span>}
                  </div>
                  <button className="plane-icon-btn" onClick={() => setSidebarCollapsed((c) => !c)}>
                    <LayoutDashboard size={15} />
                  </button>
                </div>
                {!sidebarCollapsed && (
                  <div className="plane-sidebar-quick">
                    <button className="plane-create-btn" onClick={() => navigate('/console/planning')}>
                      <CirclePlus size={15} /><span>新建任务</span>
                    </button>
                  </div>
                )}
                {!sidebarCollapsed && <div className="plane-sidebar-section-label">工作区</div>}
                <nav className="plane-nav-tree">
                  {visibleUserPages.map((page) => (
                    <button key={page.route} className={clsx('plane-nav-item', activeNav === page.route && 'plane-nav-item--active')} onClick={() => navigate(page.route)} title={page.label}>
                      <page.icon size={15} strokeWidth={1.8} />
                      {!sidebarCollapsed && <span>{page.label}</span>}
                    </button>
                  ))}
                </nav>
                {isAdmin && (<>
                  {!sidebarCollapsed && <div className="plane-sidebar-section-label">系统管理</div>}
                  <nav className="plane-nav-tree">
                    {adminPages.map((page) => (
                      <button key={page.route} className={clsx('plane-nav-item', activeNav === page.route && 'plane-nav-item--active')} onClick={() => navigate(page.route)} title={page.label}>
                        <page.icon size={15} strokeWidth={1.8} />
                ...
                <div className="plane-sidebar-footer">
                  {!sidebarCollapsed && (
                    <div className="plane-sidebar-stats">
                      <span className="plane-stat-item">任务 {runtime?.datasetCount ?? 0}</span>
                      <span className="plane-stat-dot">·</span>
                      <span className="plane-stat-item">队列 {runtime?.queueDepth ?? 0}</span>
                    </div>
                  )}
                  <div className="plane-sidebar-user">
                    <Tag color={isAdmin ? 'green' : 'blue'} size="small">{isAdmin ? '管' : '用'}</Tag>
                    {!sidebarCollapsed && <span className="plane-user-email">{user.email}</span>}
                    <button className="plane-icon-btn" onClick={() => void handleLogout()}><LogOut size={14} /></button>
                  </div>
                </div>
              </div>
              <div className="plane-main">
                <div className="plane-topbar">
                  <div className="plane-topbar-left">
                    <span className="plane-topbar-title">{visiblePages.find((p) => p.route === activeNav)?.label ?? '工作台'}</span>
                    <span className="plane-topbar-caption">{visiblePages.find((p) => p.route === activeNav)?.caption ?? ''}</span>
                  </div>
                  <div className="plane-topbar-actions">
