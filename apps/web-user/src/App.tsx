  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            <div className="dtus-shell">
              <div className="dtus-module-bar">
                <div className="dtus-module-logo"><Avatar color="blue" size="small">L</Avatar></div>
                <nav className="dtus-module-nav">
                  {visibleUserPages.map((page) => (
                    <button key={page.route} className={clsx('dtus-module-btn', activeNav === page.route && 'dtus-module-btn--active')} onClick={() => navigate(page.route)} title={page.label}>
                      <page.icon size={20} strokeWidth={1.7} />
                    </button>
                  ))}
                  {isAdmin && (<><div className="dtus-module-divider" />{adminPages.map((page) => (<button key={page.route} className={clsx('dtus-module-btn', activeNav === page.route && 'dtus-module-btn--active')} onClick={() => navigate(page.route)} title={page.label}><page.icon size={20} strokeWidth={1.7} /></button>))}</>)}
                </nav>
                <div className="dtus-module-footer">
                  <button className="dtus-module-btn" onClick={() => void handleLogout()} title="退出"><LogOut size={18} strokeWidth={1.7} /></button>
                </div>
              </div>
              <div className={clsx('dtus-nav-drawer', sidebarCollapsed && 'dtus-nav-drawer--collapsed')}>
                <div className="dtus-nav-header">
                  <div className="dtus-nav-title">{visiblePages.find((p) => p.route === activeNav)?.label ?? '工作台'}</div>
                  <button className="dtus-module-btn" onClick={() => setSidebarCollapsed((c) => !c)}><LayoutDashboard size={16} /></button>
                </div>
                <div className="dtus-nav-section-label">业务模块</div>
                <nav className="dtus-nav-list">
                  {visibleUserPages.map((page) => (<button key={page.route} className={clsx('dtus-nav-item', activeNav === page.route && 'dtus-nav-item--active')} onClick={() => navigate(page.route)}><page.icon size={16} strokeWidth={1.7} /><span>{page.label}</span></button>))}
                </nav>
                {isAdmin && (<><div className="dtus-nav-section-label">系统设置</div><nav className="dtus-nav-list">{adminPages.map((page) => (<button key={page.route} className={clsx('dtus-nav-item', activeNav === page.route && 'dtus-nav-item--active')} onClick={() => navigate(page.route)}><page.icon size={16} strokeWidth={1.7} /><span>{page.label}</span></button>))}</nav></>)}
                <div className="dtus-nav-stats">
                  <div className="dtus-stat-row"><span>任务数</span><strong>{runtime?.datasetCount ?? 0}</strong></div>
                  <div className="dtus-stat-row"><span>队列</span><strong>{runtime?.queueDepth ?? 0}</strong></div>
                </div>
              </div>
              <div className="dtus-main">
                <div className="dtus-header">
                  <div className="dtus-breadcrumb">
                    <span className="dtus-breadcrumb-root">数据工厂</span>
                    <span className="dtus-breadcrumb-sep">/</span>
                    <span className="dtus-breadcrumb-current">{visiblePages.find((p) => p.route === activeNav)?.label ?? '工作台'}</span>
                  </div>
                  <div className="dtus-header-actions">
                    <Tag color={isAdmin ? 'green' : 'blue'}>{isAdmin ? '管理员' : '普通用户'}</Tag>
                    <Text className="text-sm">{user.email}</Text>
                    <Button size="small" icon={<CirclePlus size={14} />} theme="solid" type="primary" onClick={() => navigate('/console/planning')}>新建任务</Button>
                    <Button size="small" icon={<RefreshCw size={14} />} loading={workspaceLoading} onClick={() => void loadBootstrap('控制台数据已刷新')} />
                  </div>
                </div>
                <div className="dtus-content">
                  {trustSignal ? (<div className="mb-4"><TrustSignalCard signal={trustSignal} onDismiss={() => setTrustSignal(null)} onNavigate={(route) => navigate(route)} /></div>) : null}
                  <Routes>
