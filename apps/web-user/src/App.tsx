  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            <div className="twenty-shell">
              <div className={clsx('twenty-drawer', sidebarCollapsed && 'twenty-drawer--collapsed')}>
                <div className="twenty-drawer-top">
                  <div className="twenty-logo-row">
                    <Avatar color="blue" size="extra-small">L</Avatar>
                    {!sidebarCollapsed && <span className="twenty-logo-name">数据工厂</span>}
                    <button className="twenty-icon-btn" onClick={() => setSidebarCollapsed((c) => !c)}><LayoutDashboard size={14} /></button>
                  </div>
                </div>
                <nav className="twenty-nav">
                  {visibleUserPages.map((page) => (
                    <button key={page.route} className={clsx('twenty-nav-item', activeNav === page.route && 'twenty-nav-item--active')} onClick={() => navigate(page.route)} title={page.label}>
                      <page.icon size={15} strokeWidth={1.7} />
                      {!sidebarCollapsed && <span>{page.label}</span>}
                    </button>
                  ))}
                  {isAdmin && (<>
                    <div className="twenty-nav-spacer" />
                    {adminPages.map((page) => (<button key={page.route} className={clsx('twenty-nav-item twenty-nav-item--admin', activeNav === page.route && 'twenty-nav-item--active')} onClick={() => navigate(page.route)} title={page.label}><page.icon size={15} strokeWidth={1.7} />{!sidebarCollapsed && <span>{page.label}</span>}</button>))}
                  </>)}
                </nav>
                <div className="twenty-drawer-bottom">
                  {!sidebarCollapsed ? (
                    <div className="twenty-user-row">
                      <Avatar color="blue" size="extra-small">{user.email[0]?.toUpperCase()}</Avatar>
                      <div className="twenty-user-info">
                        <span className="twenty-user-email">{user.email}</span>
                        <Tag color={isAdmin ? 'green' : 'blue'} size="small">{isAdmin ? '管理员' : '普通用户'}</Tag>
                      </div>
                      <button className="twenty-icon-btn" onClick={() => void handleLogout()} title="退出"><LogOut size={13} /></button>
                    </div>
                  ) : (
                    <button className="twenty-icon-btn" onClick={() => void handleLogout()} title="退出"><LogOut size={13} /></button>
                  )}
              ...
              <div className="twenty-main">
                <div className="twenty-topnav">
                  <div className="twenty-topnav-left">
                    <span className="twenty-page-title">{visiblePages.find((p) => p.route === activeNav)?.label ?? '工作台'}</span>
                  </div>
                  <div className="twenty-topnav-actions">
                    <Button size="small" icon={<RefreshCw size={13} />} loading={workspaceLoading} theme="borderless" onClick={() => void loadBootstrap('控制台数据已刷新')} />
                    <button className="twenty-create-btn" onClick={() => navigate('/console/planning')}>
                      <CirclePlus size={14} /><span>新建任务</span>
                    </button>
                  </div>
                </div>
                <div className="twenty-pipeline-bar">
                  {['主题结构','问题生成','答案内容','质量评估','导出交付'].map((stage, i) => (
                    <div key={i} className="twenty-pipeline-step">
                      <span className="twenty-pipeline-num">{i + 1}</span>
                      <span className="twenty-pipeline-label">{stage}</span>
                    </div>
                  ))}
                  <div className="twenty-pipeline-stats">
                    <span>任务 {runtime?.datasetCount ?? 0}</span>
                    <span>·</span>
                    <span>队列 {runtime?.queueDepth ?? 0}</span>
                  </div>
                </div>
