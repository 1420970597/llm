  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            {/* stable-diffusion-webui layout */}
            <div className="sdw-app">
              <div className="sdw-topbar">
                <div className="sdw-topbar-left">
                  <div className="sdw-topbar-brand" onClick={() => navigate('/console/home')}>
                    <div className="sdw-topbar-brand-icon">
                      <LayoutDashboard />
                    </div>
                    <span className="sdw-topbar-brand-text">{runtime?.config?.title || 'AI Factory'}</span>
                  </div>
                  <div className="sdw-tabs">
                    {(visiblePages || []).slice(0, 7).map((p: any) => {
                      const key = p.path || p.key;
                      const path = p.path || `/console/${p.key}`;
                      return (
                        <div key={key} className={clsx("sdw-tab", activeNav === key && "active")} onClick={() => navigate(path)}>
                          {p.label || p.name}
                        </div>
                      );
                    })}
                ...
                <div className="sdw-topbar-right">
                  <div className="sdw-topbar-btn" onClick={() => setSidebarCollapsed(!sidebarCollapsed)}>
                    <ChevronRight style={{ transform: sidebarCollapsed ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.15s' }} />
                  </div>
                  <div className="sdw-topbar-btn"><RefreshCw size={14} /></div>
                  <div className="sdw-topbar-btn" onClick={() => { if (window.confirm('确认退出登录?')) handleLogout(); }}>
                    <LogOut size={14} />
                  </div>
                  <Avatar size="extra-small" style={{ backgroundColor: '#f97316', width: 22, height: 22, fontSize: 10 }}>
                    {user?.username?.[0]?.toUpperCase() || 'U'}
                  </Avatar>
                </div>
              </div>
              <div className={clsx("sdw-workspace", sidebarCollapsed && "no-panel")}>
                <div className="sdw-panel">
                  <div className="sdw-panel-header">
                    <span className="sdw-panel-title">Parameters</span>
                    <div className="sdw-panel-close" onClick={() => setSidebarCollapsed(true)}>
                      <ChevronRight style={{ transform: 'rotate(180deg)' }} />
                    </div>
                  </div>
                  <div className="sdw-panel-nav">
                    {[
                      ...
                      <div key={item.key} className={clsx("sdw-panel-item", activeNav === item.key && "active")} onClick={() => navigate(item.path)}>
                ...
                <div className="sdw-content">
                  <div className="sdw-content-toolbar">
                    <span className="sdw-content-title">{activeNav || 'Workspace'}</span>
                    <div className="sdw-content-actions">
                      <div className="sdw-action-btn primary">
                        <CirclePlus size={12} />
                        Generate
                      </div>
                    </div>
