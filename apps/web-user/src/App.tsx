            ...
            <div className="ls-app">
              <div className="ls-header">
                <div className="ls-header-left">
                  <div className="ls-header-logo" onClick={() => navigate('/console/home')}>
                    <div className="ls-header-logo-icon">
                      <LayoutDashboard />
                    </div>
                    <span className="ls-header-logo-text">{runtime?.config?.title || 'AI Factory'}</span>
                  </div>
                  <div className="ls-header-nav">
                    {(visiblePages || []).slice(0, 6).map((p: any) => {
                      const key = p.path || p.key;
                      const path = p.path || `/console/${p.key}`;
                      return (
                        <div key={key} className={clsx("ls-header-nav-item", activeNav === key && "active")} onClick={() => navigate(path)}>
                          {p.label || p.name}
                        </div>
                      );
                    })}
                ...
                <div className="ls-header-right">
                  <div className="ls-header-btn"><RefreshCw size={16} /></div>
                  <div className="ls-header-btn" onClick={() => { if (window.confirm('确认退出登录?')) handleLogout(); }}>
                    <LogOut size={16} />
                  </div>
                  <Avatar size="small" style={{ backgroundColor: '#ff6550', cursor: 'pointer' }}>
                    {user?.username?.[0]?.toUpperCase() || 'U'}
                  </Avatar>
                </div>
              </div>
              <div className="ls-body">
                <div className={clsx("ls-sidebar", sidebarCollapsed && "collapsed")}>
                  <div className="ls-sidebar-header">
                    <span className="ls-sidebar-title">Navigation</span>
                    <div className="ls-sidebar-toggle" onClick={() => setSidebarCollapsed(!sidebarCollapsed)}>
                      <ChevronRight style={{ transform: sidebarCollapsed ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.2s' }} />
                    </div>
                  </div>
                  <div className="ls-sidebar-nav">
                    {[
                      ...
                      <div
                        key={item.key}
                        className={clsx("ls-sidebar-item", activeNav === item.key && "active")}
