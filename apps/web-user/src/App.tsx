        ...
        element={
          user ? (
            <Layout className="app-layout">
              {/* Metabase-style: fixed top AppBar with branding + user actions */}
              <Header className="console-header-shell metabase-appbar" style={{ padding: 0, position: 'fixed', inset: '0 0 auto 0', zIndex: 50 }}>
                <div className="metabase-appbar-inner">
                  <div className="flex items-center gap-3">
                    <Avatar color="blue" size="small">L</Avatar>
                    <span className="metabase-brand">企业数据工厂</span>
                  </div>
                  {/* Top tab navigation */}
                  <nav className="metabase-tabs">
                    {visibleUserPages.map((page) => (
                      <button
                        key={page.route}
                        className={clsx('metabase-tab', activeNav === page.route && 'metabase-tab-active')}
                    ...
                    {isAdmin && adminPages.map((page) => (
                      <button
                        key={page.route}
                        className={clsx('metabase-tab metabase-tab-admin', activeNav === page.route && 'metabase-tab-active')}
                        onClick={() => navigate(page.route)}
                      >
                        <page.icon size={15} strokeWidth={1.8} />
                        <span>{page.label}</span>
                  ...
                  <div className="console-header-actions flex items-center gap-3">
                    ...
                    <Button icon={<RefreshCw size={15} />} loading={workspaceLoading} theme="borderless" onClick={() => void loadBootstrap('控制台数据已刷新')} />
                    <Button icon={<LogOut size={15} />} theme="borderless" onClick={() => void handleLogout()} />
                  </div>
                </div>
              </Header>
              {/* No sidebar — full-width content below top nav */}
              <Layout style={{ paddingTop: 56 }}>
                <Content className="metabase-content">
                  {trustSignal ? (
                    <div className="mb-4">
