        ...
        element={
          user ? (
            <div className="refine-shell">
              {/* Refine-style collapsible sider 200px */}
              <aside className={clsx('refine-sider', sidebarCollapsed && 'refine-sider--collapsed')}>
                <div className="refine-sider-logo">
                  <Avatar color="blue" size="extra-small">L</Avatar>
                  {!sidebarCollapsed && <span className="refine-sider-logo-text">企业数据工厂</span>}
                </div>

                <nav className="refine-sider-nav">
                  <div className="refine-sider-section-label">{!sidebarCollapsed && '业务'}</div>
                  {visibleUserPages.map((page) => (
                    <button
                      key={page.route}
                      className={clsx('refine-sider-item', activeNav === page.route && 'refine-sider-item--active')}
                  ...
                  {isAdmin && (
                    <>
                      <div className="refine-sider-section-label">{!sidebarCollapsed && '系统'}</div>
                      {adminPages.map((page) => (
                        <button
                          key={page.route}
                          className={clsx('refine-sider-item', activeNav === page.route && 'refine-sider-item--active')}
                ...
                <button
                  className="refine-sider-collapse-btn"
              ...
              {/* Main area */}
              <div className={clsx('refine-main', sidebarCollapsed && 'refine-main--collapsed')}>
                {/* Header */}
                <header className="refine-header">
                  ...
                  <div className="refine-header-right">
                    <Text className="refine-user-email">{user.email}</Text>
                    <Button size="small" icon={<RefreshCw size={13} />} loading={workspaceLoading} onClick={() => void loadBootstrap('控制台数据已刷新')} />
                    <Button size="small" icon={<LogOut size={13} />} onClick={() => void handleLogout()}>退出</Button>
                  </div>
                </header>

                {/* Breadcrumb */}
                <div className="refine-breadcrumb">
                  <span className="refine-breadcrumb-root">控制台</span>
                  <span className="refine-breadcrumb-sep">/</span>
                  <span className="refine-breadcrumb-current">
                    {visiblePages.find((p) => p.route === activeNav)?.label ?? '工作台'}
                  </span>
                </div>
                ...
                <div className="refine-toolbar">
                  ...
                  {runtime && (
                    <span className="refine-toolbar-stat">
                      共 {runtime.datasetCount} 任务 · 队列 {runtime.queueDepth}
                    </span>
                  )}
                </div>

                {/* Content */}
                <main className="refine-content">
                  {trustSignal ? (
                    <div className="mb-4">
