  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            {/* vercel-commerce layout */}
            <div className="vc-app">
              <div className="vc-navbar">
                <div className="vc-navbar-left">
                  <div className="vc-navbar-brand" onClick={() => navigate('/console/home')}>
                    <div className="vc-navbar-brand-icon">
                      <LayoutDashboard />
                    </div>
                    <span className="vc-navbar-brand-text">{runtime?.config?.title || 'AI Factory'}</span>
                  </div>
                  <div className="vc-navbar-links">
                    {(visiblePages || []).slice(0, 5).map((p: any) => {
                      ...
                      return (
                        <div key={key} className={clsx("vc-navbar-link", activeNav === key && "active")} onClick={() => navigate(path)}>
                          {p.label || p.name}
                        </div>
                      );
                    })}
                ...
                <div className="vc-navbar-center">
                  <input className="vc-navbar-search" placeholder="Search for tasks..." readOnly />
                </div>
                <div className="vc-navbar-right">
                  <div className="vc-navbar-btn"><RefreshCw size={18} /></div>
                  <div className="vc-navbar-btn" onClick={() => { if (window.confirm('确认退出登录?')) handleLogout(); }}>
                    <LogOut size={18} />
                  </div>
                  <Avatar size="small" style={{ backgroundColor: '#111', cursor: 'pointer' }}>
              ...
              <div className="vc-body">
                <div className="vc-sidebar">
                  <div className="vc-sidebar-section">
                    <div className="vc-sidebar-title">Pages</div>
                    {[
                      ...
                      <div key={item.key} className={clsx("vc-sidebar-link", activeNav === item.key && "active")} onClick={() => navigate(item.path)}>
                        {item.label}
                      </div>
                    ))}
                  </div>
                  {isAdmin && (adminPages || []).length > 0 && (
                    <div className="vc-sidebar-section">
                      <div className="vc-sidebar-title">Admin</div>
                      {(adminPages || []).map((p: any) => {
                        const key = p.path || p.key;
                        const path = p.path || `/console/${p.key}`;
                        return (
                          <div key={key} className={clsx("vc-sidebar-link", activeNav === key && "active")} onClick={() => navigate(path)}>
                            {p.label || p.name}
                          </div>
                        );
                      })}
