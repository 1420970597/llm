        ...
        element={
          user ? (
            <div className="app-layout">
              {/* Budibase-style: leftmost dark icon bar */}
              <nav className="bb-icon-bar">
                <div className="bb-icon-bar-logo">L</div>
                {visibleUserPages.map((page) => (
                  <button
                    key={page.route}
                    className={clsx('bb-icon-bar-btn', { active: activeNav === page.route })}
                ...
                {isAdmin ? (
                  <>
                    <div className="bb-icon-bar-spacer" />
                    {adminPages.map((page) => (
                      <button
                        key={page.route}
                        className={clsx('bb-icon-bar-btn', { active: activeNav === page.route })}
                ...
                <button className="bb-icon-bar-btn" onClick={() => void handleLogout()} title="退出">
                  <LogOut size={18} />
                </button>
              </nav>
              {/* Second nav panel */}
              <aside className="bb-nav-sidebar">
                <div className="bb-nav-header">
                  <div className="bb-nav-app-name">企业数据工厂</div>
                  <div className="bb-nav-app-sub">{isAdmin ? '管理员' : '普通用户'} · {user.email}</div>
                </div>
                <div className="bb-nav-section-title">业务导航</div>
                {visibleUserPages.map((page) => (
                  <button
                    key={page.route}
                    className={clsx('bb-nav-item', { active: activeNav === page.route })}
                    onClick={() => navigate(page.route)}
                  >
                    <page.icon size={15} />
                    {page.label}
                ...
                {isAdmin ? (
                  <>
                    <div className="bb-nav-section-title">系统设置</div>
                    {adminPages.map((page) => (
                      <button
                        key={page.route}
                        className={clsx('bb-nav-item', { active: activeNav === page.route })}
                ...
                <div className="bb-nav-footer">
                  ...
                  <div className="mt-2 flex gap-2">
                    <Button theme="solid" type="primary" size="small" icon={<CirclePlus size={13} />} onClick={() => navigate('/console/planning')} style={{ flex: 1 }}>新建任务</Button>
                    <Button size="small" onClick={() => navigate('/console/tasks')} style={{ flex: 1 }}>我的任务</Button>
                  </div>
                </div>
              </aside>
              {/* Main content */}
              <div className="bb-main">
                <div className="bb-topbar">
                  <div className="bb-topbar-left">
                    <div>
                      <div className="bb-topbar-title">{visiblePages.find((p) => p.route === activeNav)?.label ?? '控制台'}</div>
                      <div className="bb-topbar-sub">{visiblePages.find((p) => p.route === activeNav)?.caption ?? ''}</div>
                    </div>
                  </div>
