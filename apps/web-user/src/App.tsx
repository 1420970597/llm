  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            <div className="tremor-layout">
              {/* Tremor Header */}
              <div className="tr-header">
                <div className="tr-header-left">
                  <Avatar color="blue" size="small">L</Avatar>
                  <div>
                    <Text strong style={{ fontSize: 16 }}>数据工厂</Text>
                    <Text className="block console-caption" style={{ fontSize: 12 }}>{visiblePages.find((page) => page.route === activeNav)?.caption ?? '仪表板'}</Text>
                  </div>
                </div>
                <div className="tr-header-right">
                  ...
                  <Button icon={<LogOut size={16} />} theme="borderless" onClick={() => void handleLogout()} />
                </div>
              </div>
              {/* Tab Navigation */}
              <div className="tr-tab-nav">
                {visibleUserPages.map((page) => (
                  <button
                    key={page.route}
                    className={clsx('tr-tab', activeNav === page.route && 'active')}
                ...
                <div className="tr-sidebar">
                  ...
                  <Nav
                    ...
                    items={[
                      ...
                      ...(isAdmin
                        ? [{ itemKey: 'admin-group', text: '管理', items: adminPages.map((page) => ({ itemKey: page.route, text: page.label, icon: <page.icon size={16} /> })) }]
                        : []),
                    ]}
                    onSelect={(data) => navigate(String(data.itemKey))}
                  />
                  <div className="tr-sidebar-stats">
                    <Card className="tr-sidebar-stats-card" bodyStyle={{ padding: 10 }}>
                      <div className="console-summary-grid">
