            ...
            <div className="mlf-app">
              <div className="mlf-header">
                <div className="mlf-header-left">
                  <div className="mlf-header-logo" onClick={() => navigate('/console/home')}>
                    <div className="mlf-header-logo-icon">
                      <LayoutDashboard />
                    </div>
                    <span className="mlf-header-logo-text">{runtime?.config?.title || 'AI Factory'}</span>
                  </div>
                  <div className="mlf-header-nav">
                    {(visiblePages || []).slice(0, 7).map((p: any) => {
                      const key = p.path || p.key;
                      const path = p.path || `/console/${p.key}`;
                      return (
                        <div key={key} className={clsx("mlf-header-nav-item", activeNav === key && "active")} onClick={() => navigate(path)}>
                          {p.label || p.name}
                        </div>
                      );
                    })}
                ...
                <div className="mlf-header-right">
                  <div className="mlf-header-btn"><RefreshCw size={16} /></div>
                  <div className="mlf-header-btn" onClick={() => { if (window.confirm('确认退出登录?')) handleLogout(); }}>
                    <LogOut size={16} />
                  </div>
                  <Avatar size="small" style={{ backgroundColor: '#0194e2', cursor: 'pointer' }}>
                    {user?.username?.[0]?.toUpperCase() || 'U'}
                  </Avatar>
                </div>
              </div>
              <div className="mlf-body">
                <div className={clsx("mlf-sidebar", sidebarCollapsed && "collapsed")}>
                  <div className="mlf-sidebar-header">
                    <span className="mlf-sidebar-title">Explorer</span>
                    <div className="mlf-sidebar-toggle" onClick={() => setSidebarCollapsed(!sidebarCollapsed)}>
                      <ChevronRight style={{ transform: sidebarCollapsed ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.2s' }} />
                    </div>
                  </div>
                  <div className="mlf-sidebar-search">
                    <input placeholder="Filter..." readOnly />
                  </div>
                  <div className="mlf-sidebar-nav">
                    {[
                      ...(visiblePages || []).map((p: any) => ({
                        key: p.path || p.key,
                        label: p.label || p.name,
                        icon: p.icon,
                        path: p.path || `/console/${p.key}`,
                      })),
                      ...
                      <div
                        key={item.key}
                        className={clsx("mlf-sidebar-item", activeNav === item.key && "active")}
                        onClick={() => navigate(item.path)}
                      >
                        {item.icon || <LayoutDashboard size={16} />}
                        <span>{item.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="mlf-content">
                  <div className="mlf-content-header">
                    <div className="mlf-content-title">{activeNav || 'Dashboard'}</div>
                    <Space>
                      <Button size="small" icon={<CirclePlus size={14} />} theme="solid" type="primary" style={{ backgroundColor: '#0194e2' }}>New</Button>
                    </Space>
                  </div>
                  <Routes>
