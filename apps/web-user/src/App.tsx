        ...
        element={
          user ? (
            <div className="react-admin-layout">
              {/* React Admin AppBar */}
              <div className="ra-appbar">
                <div className="ra-appbar-left">
                  <Button icon={<LayoutDashboard size={18} />} theme="borderless" style={{ color: '#fff' }} onClick={() => setSidebarCollapsed((c) => !c)} />
                  <span style={{ fontSize: 16, fontWeight: 600 }}>企业数据工厂</span>
                </div>
                <div className="ra-appbar-right">
                  <Button theme="light" size="small" icon={<CirclePlus size={14} />} style={{ color: '#fff', borderColor: 'rgba(255,255,255,0.3)' }} onClick={() => navigate('/console/planning')}>新建</Button>
                  <Tag color="white" style={{ color: 'var(--semi-color-primary)' }}>{isAdmin ? '管理员' : '用户'}</Tag>
                  <Text style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)' }}>{user.email}</Text>
                  <Button icon={<RefreshCw size={14} />} theme="borderless" style={{ color: '#fff' }} loading={workspaceLoading} onClick={() => void loadBootstrap('控制台数据已刷新')} />
                  <Button icon={<LogOut size={14} />} theme="borderless" style={{ color: '#fff' }} onClick={() => void handleLogout()} />
                </div>
              </div>
              {/* Body */}
              <div className="ra-body">
                ...
                <div className="ra-drawer">
                  ...
                  <div className="ra-drawer-nav">
                    <Nav
                      ...
                      items={[
                        ...
                        ...(isAdmin
                          ? [{ itemKey: 'admin', text: '管理', items: adminPages.map((page) => ({ itemKey: page.route, text: page.label, icon: <page.icon size={16} /> })) }]
                          : []),
                      ]}
                  ...
                  <div className="ra-drawer-footer">
                    <Card className="ra-drawer-footer-card" bodyStyle={{ padding: 10 }}>
                      <div className="console-summary-grid">
