        ...
        element={
          user ? (
            <div className="dify-layout">
              {/* Dify Top Header - Full width navigation bar */}
              <div className="dify-header">
                <div className="dify-header-left">
                  <Avatar color="blue" size="small">L</Avatar>
                  <div className="dify-brand-text">
                    <div className="font-semibold text-sm">企业数据工厂</div>
                  </div>
                </div>
                <div className="dify-header-nav">
                  {visibleUserPages.map((page) => (
                    <div
                      key={page.route}
                      className={`dify-nav-item ${activeNav === page.route ? 'dify-nav-active' : ''}`}
                  ...
                  {isAdmin
                    ? adminPages.map((page) => (
                        <div
                          key={page.route}
                          className={`dify-nav-item dify-nav-admin ${activeNav === page.route ? 'dify-nav-active' : ''}`}
                          onClick={() => navigate(page.route)}
                        >
                          <page.icon size={16} />
                          <span>{page.label}</span>
                ...
                <div className="dify-header-right">
              ...
              {/* Action Bar - Quick actions + stats */}
              <div className="dify-action-bar">
                <div className="dify-action-left">
                  <Button theme="solid" type="primary" icon={<CirclePlus size={16} />} onClick={() => navigate('/console/planning')}>新建任务</Button>
                  <Button onClick={() => navigate('/console/tasks')}>我的任务</Button>
                </div>
                <div className="dify-action-stats">
                  <div className="dify-stat-chip"><span>任务</span><Text strong>{runtime?.datasetCount ?? 0}</Text></div>
                  <div className="dify-stat-chip"><span>排队</span><Text strong>{runtime?.queueDepth ?? 0}</Text></div>
                </div>
              </div>
              {/* Main Content Area */}
              <div className="dify-content">
                {trustSignal ? (
                  <div className="mb-4">
