  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            <div className="tooljet-layout">
              {/* ToolJet Left Sidebar - IDE style */}
              <div className={`tj-sidebar ${sidebarCollapsed ? 'tj-sidebar-collapsed' : ''}`}>
                <div className="tj-sidebar-header">
                  <Avatar color="blue" size="small">L</Avatar>
                  {!sidebarCollapsed ? <span className="font-semibold text-sm">数据工厂</span> : null}
                  <Button
                ...
                {!sidebarCollapsed ? (
                  <>
                    <div className="tj-sidebar-actions">
                      <Button theme="solid" type="primary" block icon={<CirclePlus size={16} />} onClick={() => navigate('/console/planning')}>新建任务</Button>
                      <Button block onClick={() => navigate('/console/tasks')}>我的任务</Button>
                    </div>
                    <Nav
                    ...
                    <div className="tj-sidebar-footer">
                      <Card className="console-sidebar-card" bodyStyle={{ padding: 12 }}>
                        <div className="console-summary-grid">
                          <div className="console-summary-row"><span>任务总数</span><Text strong>{runtime?.datasetCount ?? 0}</Text></div>
                          <div className="console-summary-row"><span>排队中</span><Text strong>{runtime?.queueDepth ?? 0}</Text></div>
                        </div>
                      </Card>
                    </div>
                  </>
                ) : (
                  <div className="tj-collapsed-nav">
                    {visibleUserPages.map((page) => (
                      <div key={page.route} className={`tj-icon-btn ${activeNav === page.route ? 'tj-icon-active' : ''}`} onClick={() => navigate(page.route)} title={page.label}>
                        <page.icon size={18} />
                      </div>
                    ))}
              ...
              <div className="tj-main">
                <div className="tj-topbar">
                  <div className="tj-topbar-left">
                    <span className="tj-breadcrumb">{visiblePages.find((page) => page.route === activeNav)?.label ?? '首页'}</span>
                  </div>
                  <div className="tj-topbar-right">
                    <Tag color={isAdmin ? 'green' : 'blue'}>{isAdmin ? '管理员' : '普通用户'}</Tag>
                    <Text>{user.email}</Text>
                    <Button icon={<RefreshCw size={16} />} theme="borderless" loading={workspaceLoading} onClick={() => void loadBootstrap('控制台数据已刷新')} />
                    <Button icon={<LogOut size={16} />} theme="borderless" onClick={() => void handleLogout()}>退出</Button>
                  </div>
                </div>
