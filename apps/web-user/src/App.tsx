  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            {/* nextjs-subscription layout */}
            <div className="nxs-app">
              <div className="nxs-navbar">
                <div className="nxs-navbar-left">
                  <div className="nxs-navbar-brand" onClick={() => navigate('/console/home')}>
                    <div className="nxs-navbar-brand-icon">
                      <LayoutDashboard />
                    </div>
                    <span className="nxs-navbar-brand-text">{runtime?.config?.title || 'AI Factory'}</span>
                  </div>
                  <div className="nxs-navbar-links">
                    {[
                      ...(visiblePages || []).map((p: any) => ({
                        key: p.path || p.key, label: p.label || p.name, path: p.path || `/console/${p.key}`,
                      })),
                      ...
                      <div key={item.key} className={clsx("nxs-navbar-link", activeNav === item.key && "active")} onClick={() => navigate(item.path)}>
                        {item.label}
                      </div>
                    ))}
                  </div>
                </div>
                <div className="nxs-navbar-right">
                  <div className="nxs-navbar-btn"><RefreshCw size={16} /></div>
                  <div className="nxs-navbar-btn" onClick={() => { if (window.confirm('确认退出登录?')) handleLogout(); }}>
                    <LogOut size={16} />
                  </div>
                  <Avatar size="small" style={{ backgroundColor: '#fff', color: '#000', cursor: 'pointer' }}>
                    {user?.username?.[0]?.toUpperCase() || 'U'}
                  </Avatar>
                </div>
              </div>
              <div className="nxs-main">
                <div className="nxs-page-header">
                  <div className="nxs-page-title">{activeNav || 'Dashboard'}</div>
                  <div className="nxs-page-subtitle">Manage your AI data factory workspace</div>
                </div>
                <div className="nxs-content">
                  <Routes>
              ...
              <div className="nxs-footer">
                Powered by AI Data Factory
              </div>
            </div>
          ) : (
