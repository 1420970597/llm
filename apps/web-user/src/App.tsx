    ...
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            {/* gradio layout */}
            <div className="grd-app">
              <div className="grd-header">
                <div className="grd-header-left">
                  <div className="grd-header-brand" onClick={() => navigate('/console/home')}>
                    <div className="grd-header-brand-icon">
                      <LayoutDashboard />
                    </div>
                    <span className="grd-header-brand-text">{runtime?.config?.title || 'AI Factory'}</span>
                  </div>
                  <div className="grd-header-tabs">
                    {(visiblePages || []).slice(0, 6).map((p: any) => {
                      const key = p.path || p.key;
                      const path = p.path || `/console/${p.key}`;
                      return (
                        <div key={key} className={clsx("grd-header-tab", activeNav === key && "active")} onClick={() => navigate(path)}>
                          {p.label || p.name}
                        </div>
                      );
                    })}
                ...
                <div className="grd-header-right">
                  <div className="grd-header-btn" onClick={() => setSidebarCollapsed(!sidebarCollapsed)}>
                    <ChevronRight style={{ transform: sidebarCollapsed ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.2s' }} />
                  </div>
                  <div className="grd-header-btn"><RefreshCw size={16} /></div>
                  <div className="grd-header-btn" onClick={() => { if (window.confirm('确认退出登录?')) handleLogout(); }}>
                    <LogOut size={16} />
                  </div>
                  <Avatar size="extra-small" style={{ backgroundColor: '#ff7c00', width: 24, height: 24, fontSize: 11 }}>
                    {user?.username?.[0]?.toUpperCase() || 'U'}
                  </Avatar>
                </div>
              </div>
              <div className="grd-body">
                <div className={clsx("grd-sidebar", sidebarCollapsed && "collapsed")}>
                  <div className="grd-sidebar-header">
                    <span className="grd-sidebar-title">Components</span>
                    <div className="grd-sidebar-toggle" onClick={() => setSidebarCollapsed(true)}>
                      <ChevronRight style={{ transform: 'rotate(180deg)' }} />
                    </div>
                  </div>
                  <div className="grd-sidebar-nav">
                    {[
                      ...
                      <div key={item.key} className={clsx("grd-sidebar-item", activeNav === item.key && "active")} onClick={() => navigate(item.path)}>
              ...
              <div className="grd-footer">
                <span>{user?.username || 'User'} - {activeNav || 'Home'}</span>
                <span>Powered by AI Factory</span>
              </div>
            </div>
          ) : (
