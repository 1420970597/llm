        ...
        element={
          user ? (
            <Layout className="app-layout">
              <div className="nocodb-toolbar" style={{ position: 'fixed', inset: '0 0 auto 0', zIndex: 50 }}>
                <div className="nocodb-toolbar-inner">
                  <div className="nocodb-toolbar-left">
                    <Avatar color="blue" size="small">L</Avatar>
                    <span className="nocodb-toolbar-logo">企业数据工厂</span>
                    <span className="nocodb-toolbar-sep">/</span>
                    <span className="nocodb-toolbar-view">{visiblePages.find((page) => page.route === activeNav)?.label ?? '概览'}</span>
                  </div>
                  <div className="nocodb-toolbar-center">
                    {visibleUserPages.map((page) => (
                      <button
                        key={page.route}
                        className={clsx('nocodb-view-tab', activeNav === page.route && 'nocodb-view-tab-active')}
                  ...
                  <div className="nocodb-toolbar-right">
              ...
              <Layout style={{ paddingTop: 44 }}>
                <Sider className="nocodb-sidebar" style={{ position: 'fixed', top: 44, left: 0, bottom: 0, border: 'none', width: 240, zIndex: 30 }}>
                  <div className="nocodb-sidebar-inner">
                    <div className="nocodb-sidebar-section">
                      <div className="nocodb-sidebar-section-title">业务导航</div>
                      {visibleUserPages.map((page) => (
                        <button
                          key={page.route}
                          className={clsx('nocodb-sidebar-item', activeNav === page.route && 'nocodb-sidebar-item-active')}
                          onClick={() => navigate(page.route)}
                        >
                          <page.icon size={16} />
                          <span>{page.label}</span>
                    ...
                    {isAdmin ? (
                      <div className="nocodb-sidebar-section">
                        <div className="nocodb-sidebar-section-title">系统设置</div>
                        {adminPages.map((page) => (
                          <button
                            key={page.route}
                            className={clsx('nocodb-sidebar-item', activeNav === page.route && 'nocodb-sidebar-item-active')}
                    ...
                    <div className="nocodb-sidebar-footer">
                      <div className="nocodb-sidebar-stat"><span>任务</span><Text strong>{runtime?.datasetCount ?? 0}</Text></div>
                      <div className="nocodb-sidebar-stat"><span>队列</span><Text strong>{runtime?.queueDepth ?? 0}</Text></div>
                    </div>
                  </div>
                </Sider>
                <Layout style={{ marginLeft: 240 }}>
                  <Content className="nocodb-content">
                    {trustSignal ? (
                      <div className="mb-4">
