  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          user ? (
            <div className="gitea-layout">
              {/* Gitea Top Navigation */}
              <div className="gt-topnav">
                <div className="gt-topnav-left">
                  <Avatar color="blue" size="extra-small">L</Avatar>
                  <span style={{ fontSize: 14, fontWeight: 600, color: '#fff' }}>数据工厂</span>
                  <div className="gt-topnav-links">
                    {visibleUserPages.slice(0, 4).map((page) => (
                      <button
                        key={page.route}
                        className={clsx('gt-topnav-link', activeNav === page.route && 'active')}
                ...
                <div className="gt-topnav-right">
                  <Button icon={<CirclePlus size={16} />} theme="solid" type="primary" size="small" onClick={() => navigate('/console/planning')}>新建</Button>
                  <Tag color={isAdmin ? 'green' : 'blue'} style={{ fontSize: 11 }}>{isAdmin ? '管理员' : '用户'}</Tag>
                  <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>{user.email}</Text>
                  <Button icon={<RefreshCw size={14} />} theme="borderless" size="small" style={{ color: 'rgba(255,255,255,0.6)' }} loading={workspaceLoading} onClick={() => void loadBootstrap('控制台数据已刷新')} />
                  <Button icon={<LogOut size={14} />} theme="borderless" size="small" style={{ color: 'rgba(255,255,255,0.6)' }} onClick={() => void handleLogout()} />
                </div>
              </div>
              {/* Tab Bar */}
              <div className="gt-tab-bar">
                {visibleUserPages.map((page) => (
                  <button
                    key={page.route}
                    className={clsx('gt-tab', activeNav === page.route && 'active')}
                    onClick={() => navigate(page.route)}
                  >
                    {page.label}
                  </button>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  <Button icon={<LayoutDashboard size={14} />} theme="borderless" size="small" onClick={() => setSidebarCollapsed((c) => !c)} />
                </div>
              </div>
              {/* Body */}
              <div className="gt-body">
                <div className="gt-sidebar">
                  ...
                  />
                  <div className="gt-sidebar-stats">
                    <Card className="gt-sidebar-stats-card" bodyStyle={{ padding: 10 }}>
                      <div className="console-summary-grid">
