...
- **端口冲突处理：禁止影响其他业务。** 启动容器/服务前先探测端口占用（`ss -tln`）。若端口被占用，**更换本项目的映射端口**（改 `deployments/compose/docker-compose.yml` 的 `ports`，或换 compose 项目名），**绝对不得停止、重启或改动任何已占用该端口的其他业务**。本机是多业务共存环境，其他项目的容器不属于本项目，不得为其让路而中断。
- 本项目 compose 一律加 `-p llm` 指定项目名，避免与本机其他项目（如 `llmreview`）混淆。

### 本机端口事实（部署前必看）

本项目只有 web-user 映射宿主机端口，api/worker 仅在 compose 网络内。

| 服务 | 宿主机映射 | 状态 |
|---|---|---|
| web-user | `3210:80` | 3210 空闲 |
| api | 无（仅容器内 8080） | 不受宿主机占用影响 |
| worker | 无（仅容器内 8081） | 不受宿主机占用影响 |

**宿主机 8080 已被 `map_army_backend` 占用（长期运行），但与本项目无关** —— web-user 的 nginx 经 compose 服务名 `api:8080` 在容器网络内访问，不经过宿主机端口。同理 8081 被其他项目占用也不影响 worker。因此本项目**无需**因 8080/8081 而改端口。
...
- 9. 在每次commit之后，需要获取远程仓库的issue，如有问题，则第一时间进行分析、开发、测试，并在issue中回复问题的原因、处理方式，最后再继续原有的开发流程。
- 10. 在完成某一个功能后，必须提交pr合并。**注**：即功能开发完毕、测试通过后，需推送特性分支并创建 PR，待 CI 检查通过后合并入 main，不得将已完成的功能长期留在未合并分支上。
