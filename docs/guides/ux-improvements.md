...
| 真实浏览器逐页截图 | 24 张，见 `docs/assets/ux-after/`（含 `manifest.json` 逐页 URL 与 md5） |
| 模板预填 | 实测点「行业研究问答」→ 主题自动填「行业研究」、规模填 50 |
| 「下一步」按钮 | 实测 5 个阶段页全链路正确：问题生成 → 答案内容 → 质量评分 → 导出交付 → 建议 |
...
### 4.1 截图事故（如实记录，含一次错误定因）

首版截图脚本只截图、**不校验**，产出了一张错图：`01-login.png` 截到的不是登录页，
而是**登录后**的任务页（md5 与 `04-tasks.png` 完全相同）。因此首版 24 张中实际只有
23 张不同图，重复组为 `01-login = 04-tasks`。

**真实根因**：脚本顺序错了——先登录、后去拍登录页，而 `/login` 在已登录态下会重定向到
`/console/tasks`，于是「登录页」拍成了任务页。不是什么环境异常。

**需要记录的自我修正**：首次排查时，我根据「肉眼读图」认定是 `04-tasks.png` 截成了登录页，
并给出了「截图进行中容器被并发重建、会话失效被重定向」的根因。这两个说法**都是错的**：

1. `04-tasks.png` 一直是正确的任务页；
2. 那个「并发重建」的故事是我根据时间线**编造的**，没有任何证据支持。

直到用 `git show HEAD:...` 比对提交版 md5 才纠正。教训有两条：

- **不定无证据的因**。听起来合理的时间线不等于证据；
- **不要靠肉眼看图下结论**。md5 与 DOM 断言是确定性的，肉眼读图不是。

修正后的脚本（`screenshot2.mjs`）加了三道约束：

1. 登录页在**登录前**捕获，避免被重定向掉包；
2. 每页截图前断言**不在登录页**（检测密码框）且正文 ≥60 字，失败即非零退出；
3. 输出 `manifest.json`（每页 URL / 正文片段 / md5）并做唯一性校验。

重跑后：24 张、23 唯一，唯一重复组为预期内的 `06-evaluation = 21-eval-dimensions`
（质量评估默认页签即「维度管理」），且每张文件 md5 与 `manifest.json` 一致、
无任何页面落回登录页。
...
4. **下载校验缺浏览器端到端验证**：交付物下载按钮需先有导出的 artifact，演示数据集未走到
   导出阶段；当前证据为单元测试（6/6）+ 构建通过，**不含**浏览器 e2e。
5. **未合并**：本分支等待评审，未执行 `git push`，未创建 PR。
