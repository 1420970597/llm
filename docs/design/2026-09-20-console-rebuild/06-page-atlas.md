...
| ID | 页面 | 原型地址 | 主要职责 |
|---|---|---|---|
| G01 | [登录](#g01) | `/login` | 邮箱登录，回到登录前访问的对象 |
| G02 | [工作台](#g02) | `/home` | 待办、近期任务和最近交付，直接进入处理对象 |
| G03 | [通知中心](#g03) | `/notifications` | 任务完成、异常和待处理提醒 |
| G04 | [账户](#g04) | `/account` | 当前身份、偏好与退出登录 |
| G05 | [帮助中心](#g05) | `/help` | 按业务问题查找操作指引 |
| G06 | [创建第一份训练数据](#g06) | `/help/first-dataset` | 从目标到交付的完整教程 |
| D01 | [数据集](#d01) | `/datasets` | 跨任务检索内容资产 |
| D02 | [仓储调度 · SFT](#d02) | `/datasets/24` | 内容、来源、质量与交付概览 |
| D03 | [样本浏览](#d03) | `/datasets/24/samples` | 按状态、方向和质量筛选样本 |
| D04 | [样本 #1024](#d04) | `/datasets/24/samples/1024` | 问题、推理、答案及评估证据 |
| D05 | [导出记录](#d05) | `/datasets/24/exports` | 查看同一数据集的每次交付 |
| D06 | [创建导出](#d06) | `/datasets/24/exports/new` | 范围、格式、字段预览及完整性检查 |
| D07 | [导出 #81](#d07) | `/datasets/24/exports/81` | 可追溯的导出配置、状态和下载 |
| T01 | [生产任务](#t01) | `/tasks` | 按状态筛选执行任务并进入详情 |
| T02 | [创建生产任务](#t02) | `/tasks/new` | 目标、结构规模、生成方案三步向导 |
| T03 | [仓储调度 · 生产任务](#t03) | `/tasks/24` | 进度、当前待办和最近运行 |
| T04 | [领域与方向](#t04) | `/tasks/24/structure` | 生成、编辑、校验并确认结构 |
| T05 | [思维标准](#t05) | `/tasks/24/standards` | 按方向管理长链思维步骤 |
| T06 | [订单分配 · 思维标准](#t06) | `/tasks/24/standards/12` | 逐步编辑、保存版本并比较历史 |
| T07 | [问题集](#t07) | `/tasks/24/questions` | 生成、难度分布及问题预览 |
| T08 | [训练产物](#t08) | `/tasks/24/output` | 根据训练类型查看 SFT 或 GRPO 产物 |
| T09 | [运行记录](#t09) | `/tasks/24/runs` | 每个阶段的执行历史、错误及恢复 |
| T10 | [运行 #104](#t10) | `/tasks/24/runs/104` | 失败影响、已完成范围和续跑入口 |
| T11 | [自动评分（历史流程）](#t11) | `/tasks/24/scoring` | 承接既有 rewards 记录，与多模型评估分开 |
| E01 | [质量评估](#e01) | `/evaluations` | 默认进入评估运行列表 |
| E02 | [新建评估](#e02) | `/evaluations/new` | 数据范围、维度和独立裁判配置 |
| E03 | [仓储调度 · 双模型评估](#e03) | `/evaluations/36` | 运行状态、配置和当前结论 |
| E04 | [逐条评分](#e04) | `/evaluations/36/scores` | 按样本、裁判、维度定位分歧 |
| E05 | [样本 #1024 · 评分证据](#e05) | `/evaluations/36/items/1024` | 多裁判理由并排对照 |
| E06 | [评估报告](#e06) | `/evaluations/36/report` | 分布、覆盖范围及改进建议 |
| C01 | [数据清洗](#c01) | `/cleaning` | 清洗运行列表与报告入口 |
| C02 | [新建清洗](#c02) | `/cleaning/new` | 阶段、规则、预览命中与执行 |
| C03 | [仓储调度 · 清洗 #18](#c03) | `/cleaning/18` | 扫描、命中、隔离及复查统计 |
| C04 | [命中明细](#c04) | `/cleaning/18/findings` | 筛选规则和处置状态，进入单条复查 |
| C05 | [命中 #7 · 人工复查](#c05) | `/cleaning/18/findings/7` | 上下文高亮及保留/隔离决策 |
| R01 | [资源库](#r01) | `/resources` | 生成模板、指令、维度、规则、关键词与字段映射 |
| R02 | [生成模板](#r02) | `/resources/templates` | 复用目标类型、规模和生成偏好 |
| R03 | [标准 SFT 模板](#r03) | `/resources/templates/1` | 查看参数、用于新任务或编辑 |
| R04 | [评估维度](#r04) | `/resources/dimensions` | 内置 58 项与自定义维度分别检索 |
| R05 | [逻辑一致性](#r05) | `/resources/dimensions/1` | 评分细则、分值与权重 |
| R06 | [清洗规则](#r06) | `/resources/rules` | 规则执行范围、动作和启用状态 |
| R07 | [拒答文本复查](#r07) | `/resources/rules/1` | 命中条件、阶段和处置配置 |
| R08 | [关键词库](#r08) | `/resources/keywords` | 匹配模式、类别及启用状态 |
| R09 | [关键词 · 对不起](#r09) | `/resources/keywords/1` | 查看和编辑单个匹配项 |
| R10 | [批量导入关键词](#r10) | `/resources/keywords/import` | 逐行解析、去重预览后导入 |
| R11 | [生成指令](#r11) | `/resources/prompts` | 按生成阶段管理提示词 |
| R12 | [仓储问题生成指令](#r12) | `/resources/prompts/1` | 系统指令、用户指令及版本 |
| R13 | [导出字段映射](#r13) | `/resources/mappings` | 按训练类型与格式管理映射 |
| R14 | [SFT · Alpaca 映射](#r14) | `/resources/mappings/1` | 源字段、目标字段和导出预览 |
| A01 | [系统管理](#a01) | `/admin` | 服务就绪、队列和配置问题 |
| A02 | [模型服务](#a02) | `/admin/providers` | 连接配置与可用性 |
| A03 | [模型服务 · Atlas](#a03) | `/admin/providers/1` | 连接、测试、模型列表及密钥管理 |
| A04 | [结果存储](#a04) | `/admin/storage` | 可用存储与默认落点 |
| A05 | [主存储](#a05) | `/admin/storage/1` | 连接配置和历史产物关联 |
| A06 | [操作记录](#a06) | `/admin/audit` | 按对象、时间和动作检索 |
| A07 | [操作记录 #91](#a07) | `/admin/audit/91` | 变更前后对比与对象跳转 |
| D08 | [仓储调度 · GRPO](#d08) | `/datasets/25` | GRPO 资产、教师评判标准与交付 |
| D09 | [GRPO 样本列表](#d09) | `/datasets/25/samples` | 问题与教师提示词的完整性 |
| D10 | [GRPO 样本 #2024](#d10) | `/datasets/25/samples/2024` | 教师提示词、奖励档位与框架引用 |
| D11 | [创建 GRPO 导出](#d11) | `/datasets/25/exports/new` | GRPO 字段预览和类型完整性 |
| D12 | [GRPO 导出 #82](#d12) | `/datasets/25/exports/82` | 教师评判训练文件下载 |
| T12 | [思维标准 · v2 快照](#t12) | `/tasks/24/standards/12/versions/2` | 只读历史版本，与当前编辑页分开 |
| E07 | [评估维度 · 运行快照](#e07) | `/evaluations/36/dimensions/logic-v1` | 本次评分使用的量表、权重和细则 |
| C06 | [GRPO 问题清洗 #19](#c06) | `/cleaning/19` | 只扫描 GRPO 问题，教师提示词适配待补 |
