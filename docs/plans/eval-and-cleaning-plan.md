...
> 冻结版本：`v1.0`（对应迁移 `0011` ~ `0018`；0011-0016 为评估/清洗核心，0017-0018 为 GRPO/SFT 一等公民表）
...
│  ┌───────────────┐ ┌──────────────┐ ┌────────────────────┐  │
│  │ sql/0011-0018 │ │ model/*.go   │ │ store/*_store.go   │  │
│  │ 迁移（全表）   │ │ 类型（全量）  │ │ 构造器（全量）      │  │
│  └───────────────┘ └──────────────┘ └────────────────────┘  │
│  ┌───────────────────────────┐ ┌──────────────────────────┐ │
...
## 2. 数据库迁移契约（编号区间 0011 ~ 0018）

编号区间已冻结，且 **0011 ~ 0018 已在 `main` 落地**（见 `sql/migrations/`）。
**lane 不得新增迁移文件**；若确需字段，走 issue 由父代理统一加。

区间分配：

| 编号 | 归属 | 内容 |
|---|---|---|
| 0011 | L7-L10 | 评估核心（eval_dimensions / eval_runs / ... / eval_summaries） |
| 0012 | L11-L12 | 清洗核心（cleaning_keywords / cleaning_rules / ...） |
| 0013 | L2 | chain_standards（长链思维标准步骤） |
| 0014 | L3 | questions_v2（问题 v2：x 可控 + 难度分层） |
| 0015 | L6 | export_mappings（导出字段映射） |
| 0016 | L1 | generation_runs（断点续跑游标）+ datasets 扩展 |
| 0017 | L4 | grpo_prompts（教师评判提示词） |
| 0018 | L5 | sft_records（思维链 + 答案） |
