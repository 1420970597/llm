...
| 端到端验收 | 首轮 **69/2/0** → 逐项修复后 **73 通过 / 0 失败 / 0 跳过**（PR #131/#133 合并后的最终实测） |
...
#### 修复后的最终实测（PR #133 合并后，eval run 22）

同一套断言、同一批裁判，对比 PR #133（裁判结构化调用重试）前后：

| | run 21（修复前） | run 22（修复后） |
| --- | --- | --- |
| 状态 | `partial_failed` | **`completed`** |
| `error_summary` | `6 次打分失败` | **空** |
| 打分条数 | 47/48 | **48/48**（8 项 × 3 维度 × 2 裁判） |
| 含裁判理由 | — | **48/48** |

**每个 llm 对该数据集的整体分数**（`eval_summaries` 的 `judge` 粒度）：

```text
judge 11 (hy3-judge / global:hy3)           4.208   24 条
judge  7 (gpt-5.6-sol-judge / global:hy4-preview)  4.083   24 条
overall                                     4.069    8 条   <- 跨 llm 汇总
judgeAgreement                              0.2866
```

报告给出的中文结论（真实文本，非模板）：

> 整体质量良好（归一化得分 77/100），但存在明显短板维度。建议按维度筛选后再使用。
> 最弱维度是「答案完整性」（答案质量类），均分 3.38，归一化得分 0.59，低于整体归一化水平 0.77。

**即：修复后评估能以 `completed` 收敛，报告给出最终质量结论** —— 这正是「多 LLM 互评」稳定可用的标志。

#### 关于 run 21 的状态为 `partial_failed`（这是**正确**行为，不是缺陷）
...
**逐项修复后，最终验收为 73 通过 / 0 失败 / 0 跳过（PR #131/#133 合并后实测）。**
