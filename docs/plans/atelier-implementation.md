...
| 2026-09-21 | T18 | T18 尚未全部交付（已交付迁移 0036 与模型判据；比较 store、项目命令 API、/compare 页面与扩量预填仍待续）| **已交付**：迁移 0036（comparison_baselines / comparison_adoptions / project_adopted_batches，含「两侧不能相同」的 CHECK、只追加的采用依据、可更新的采用指针）、`internal/model/comparison.go`（口径判定、配对统计、可比性标签与限定、采用校验）与 `internal/model/comparison_test.go`。**未交付**：比较 store（按 baseline 拉取两侧配对观测）、项目命令 API（创建基准/采用方案）、`/compare` 页面与 `/runs/new` 版本预填 |
---
