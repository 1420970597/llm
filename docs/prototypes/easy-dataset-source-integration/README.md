...
flowchart LR
  subgraph EXT["外部（独立部署，不进本仓库构建）"]
    E1["PDF / DOCX / EPUB 解析"] --> E2["章节感知递归分块"]
    E2 --> E3["块摘要 + 领域标签树"]
  end
...
- **中栏下**：领域标签树。带「已关联 / 未关联」徽标 —— 未关联就是覆盖矩阵里的缺口，在两处一致可见。
...
- **字段映射表**是核心：5 个产物字段 → 5 个本项目字段，全部显式。特别地 `chunks[].hash → source_chunks.content_hash` 标注「幂等去重依据」—— 把调研里的三层幂等**做成用户可见的契约**。
- **预检结果**给出三类可操作结论：214 条通过 / 3 条缺 tagPath（待人工，附具体 chunk 编号）/ 2 条 hash 重复（将跳过）。
...
│ 方案库     │ ▸ 思维标准 v2 │  为版本，只影响之后的批次。           │  [章节感知递归分块 ▾] │
