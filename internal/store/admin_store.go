...
func (s *AdminStore) ListProviders(ctx context.Context) ([]model.ModelProvider, error) {
  rows, err := s.db.Query(ctx, `
    SELECT id, name, base_url, model, provider_type, max_concurrency, timeout_seconds, is_active, api_key_masked, created_at, updated_at
  ...
  for rows.Next() {
    ...
    if err := rows.Scan(&item.ID, &item.Name, &item.BaseURL, &item.Model, &item.ProviderType, &item.MaxConcurrency, &item.TimeoutSeconds, &item.IsActive, &item.APIKeyMasked, &item.CreatedAt, &item.UpdatedAt); err != nil {
      return nil, err
    }
  ...
  if input.ID == 0 {
    err = s.db.QueryRow(ctx, `
      INSERT INTO model_providers (
        name, base_url, model, provider_type, max_concurrency, timeout_seconds, is_active, encrypted_api_key, api_key_masked
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, CASE WHEN $8 = '' THEN NULL ELSE $8 END, $9)
      RETURNING id, name, base_url, model, provider_type, max_concurrency, timeout_seconds, is_active, api_key_masked, created_at, updated_at`,
      input.Name,
      input.BaseURL,
      input.Model,
      input.ProviderType,
    ...
    ).Scan(&item.ID, &item.Name, &item.BaseURL, &item.Model, &item.ProviderType, &item.MaxConcurrency, &item.TimeoutSeconds, &item.IsActive, &item.APIKeyMasked, &item.CreatedAt, &item.UpdatedAt)
  } else {
    err = s.db.QueryRow(ctx, `
      UPDATE model_providers SET
        ...
        timeout_seconds = $7,
        is_active = $8,
        encrypted_api_key = COALESCE(NULLIF($9, ''), encrypted_api_key),
        api_key_masked = CASE WHEN $10 = '' THEN api_key_masked ELSE $10 END,
        updated_at = NOW()
      WHERE id = $1
      RETURNING id, name, base_url, model, provider_type, max_concurrency, timeout_seconds, is_active, api_key_masked, created_at, updated_at`,
      input.ID,
