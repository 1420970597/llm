...
func (s *DatasetStore) ResolveProvider(ctx context.Context, providerID int64) (string, string, string, string, error) {
  var baseURL, modelName, providerType, encryptedKey string
  err := s.db.QueryRow(ctx, `
    SELECT base_url, model, provider_type, COALESCE(encrypted_api_key, '')
    FROM model_providers
    WHERE id = $1`, providerID,
  ).Scan(&baseURL, &modelName, &providerType, &encryptedKey)
  if err != nil {
    return "", "", "", "", err
  }
