...
func (s *ArtifactStore) Runtime(ctx context.Context) (model.RuntimeStatus, error) {
	...
	counts = append(counts,
		struct {
			query string
			dest  *int
		}{`SELECT COUNT(*) FROM datasets`, &status.DatasetCount},
		struct {
			query string
			dest  *int
		}{`SELECT COUNT(*) FROM questions`, &status.QuestionCount},
		struct {
			query string
			dest  *int
		}{`SELECT COUNT(*) FROM reasoning_records`, &status.ReasoningCount},
		struct {
			query string
			dest  *int
		}{`SELECT COUNT(*) FROM reward_records`, &status.RewardCount},
		struct {
			query string
			dest  *int
		}{`SELECT COUNT(*) FROM artifacts`, &status.ArtifactCount},
	)
